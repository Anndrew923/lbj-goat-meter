...
export default function BattleCard({
  ...
  const availableHeight = Math.max(0, containerSize.height - BUTTON_GROUP_RESERVE)
  const scale =
    containerSize.width > 0 && containerSize.height > 0
      ? Math.min(1, containerSize.width / CARD_SIZE, availableHeight / CARD_SIZE)
      : 1
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center gap-y-6">
          ...
          {/* Layer 3: 內容（底部 pb-12 留呼吸空間，QR 完整浮現） */}
          <div className="relative z-20 flex flex-col flex-1 min-h-0 p-5 pb-12">
            ...
            <div className="text-center uppercase flex-shrink-0 overflow-hidden mb-3 relative">
              ...
              <h2
                className={`relative text-sm tracking-[0.2em] text-white/60 mb-1 ${!isTitleUppercase ? 'tracking-[0.1em]' : ''}`}
              >
                {battleSubtitle}
              </h2>
              <h1
            ...
            <div className="flex items-center gap-3 flex-shrink-0 mb-3">
              <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white/20 bg-white/10 flex-shrink-0">
                {photoURL ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-xl text-white/60">?</div>
                )}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-white font-bold truncate text-sm">{displayName || t('anonymousWarrior')}</p>
                <p
                  className="text-sm truncate"
                  style={{ color: teamColors.primary }}
                  title={t('supporting_team', { team: teamLabel })}
                >
                  {teamLabel ? String(teamLabel).toUpperCase() : t('supporting_team', { team: teamLabel })}
                </p>
              </div>
            </div>
            ...
            {/* 底部：地區 + 排名 + QR 區（至少 20px 保護區） */}
            <div className="mt-auto pt-5 flex flex-wrap items-end justify-between gap-2 border-t border-white/10 px-1">
              <div className="flex flex-col min-w-0">
                <span className="text-white/80 truncate text-xs" title={regionText}>
                  {regionText}
                </span>
                <span className="text-white/70 text-xs mt-0.5" title={rankLabel ?? t('rankLabel')}>
            ...
            {/* 免責聲明 */}
            <p className="text-[8px] text-white/40 mt-2 text-center leading-tight" aria-hidden>
