...
export function AuthProvider({ children }) {
      ...
      coolingTimeoutRef.current = setTimeout(() => {
        ...
        profileUnsubscribeRef.current = onSnapshot(
          ...
          (err) => {
            if (import.meta.env.DEV) {
              console.warn('[AuthContext] profile onSnapshot 錯誤:', err?.message ?? err)
            }
            setProfileLoading(false)
          }
        )
        ...
        const tryFetchPremium = (retryCount = 0) => {
          fetchIsPremium(user.uid)
            .then((isPremium) => {
              setCurrentUser((prev) => (prev?.uid === user.uid ? { ...prev, isPremium } : prev))
            })
            .catch((err) => {
              if (import.meta.env.DEV) {
                console.warn('[AuthContext] 取得 isPremium 失敗:', err?.message ?? err)
              }
              const isOffline = err?.message?.includes('offline') ?? false
              if ((isOffline || err?.code === 'unavailable') && retryCount < IS_PREMIUM_RETRY_MAX) {
  ...
  const loginWithGoogle = useCallback(async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.warn('[AuthContext] signInWithPopup 失敗:', err?.code ?? err?.message)
      throw err
    }
  }, [])
